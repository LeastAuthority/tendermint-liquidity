...
func TestFuzzGenesisState_raw(t *testing.T) {
	_, panics, _ := fleece.
		MustNewCrasherIterator(env, FuzzGenesisState_raw, filters...).
		TestFailingLimit(t, crashLimit)

	require.Zero(t, panics)
}

func TestFuzzGenesisState_structured(t *testing.T) {
	_, panics, _ := fleece.
		MustNewCrasherIterator(env, FuzzGenesisState_structured, filters...).
		TestFailingLimit(t, crashLimit)
